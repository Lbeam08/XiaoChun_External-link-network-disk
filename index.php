...
echo '<tr><td><b>'.$i++.'</b></td><td><a href="'.$fileurl.'" style="color: #ff6f6f;">下载</a>｜<a href="'.$viewurl.'" style="color: #ff6f6f;">查看</a></td><td><i class="fa '.type_to_icon($res['type']).' fa-fw"></i>'.$res['name'].'</td><td>'.size_format($res['size']).'</td><td><font color="blue">'.($res['type']?$res['type']:'未知').'</font></td><td>'.$res['addtime'].'</td><td>'.preg_replace('/\d+$/','*',$res['ip']).'</b></td></tr>';
}
